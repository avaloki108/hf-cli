...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('cli-package-name', {
      description:
        'fully qualified package name with scope (e.g @huggingface/hf-cli)',
      string: true,
      default: '@huggingface/hf-cli',
    })
    .option('dry-run', {
  ...
  run('git config user.name "hf-cli-robot"', dryRun);
  run('git config user.email "hf-cli-robot@google.com"', dryRun);
