...
import { debugLogger, getErrorMessage } from '@huggingface/hf-cli-core';
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to HuggingFace CLI',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: hf hooks migrate --from-claude\n\nMigrate hooks from Claude Code to HuggingFace CLI format.',
      );
    }
    await exitCli();
  },
