...
import { getPackageJson, debugLogger } from '@huggingface/hf-cli-core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of HuggingFace CLI is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `HuggingFace CLI update available! ${currentVersion} → ${latestUpdate}`;
