...
- **Error:
  ...
  - **Cause:** This error might occur if HuggingFace CLI detects the
...
- **Error: Command not found (when attempting to run HuggingFace CLI with
  `gemini`).**
  - **Cause:** HuggingFace CLI is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed HuggingFace CLI:
    - If you installed `gemini` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update HuggingFace CLI using the
      command `npm install -g @huggingface/hf-cli@latest`.
    - If you are running `gemini` from source, ensure you are using the correct
      ...
      update HuggingFace CLI, pull the latest changes from the repository, and
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, HuggingFace CLI may attempt
    operations that are restricted by your sandbox configuration, such as
    writing outside the project directory or system temp directory.
...
- **HuggingFace CLI is not running in interactive mode in "CI" environments**
  - **Issue:** The HuggingFace CLI does not enter interactive mode (no prompt
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents the HuggingFace CLI from starting in its
    interactive mode.
...
- **DEBUG mode not working from project .env file**
  - **Issue:** Setting `DEBUG=true` in a project's `.env` file doesn't enable
    debug mode for hf-cli.
  - **Cause:** The `DEBUG` and `DEBUG_MODE` variables are automatically excluded
    from project `.env` files to prevent interference with hf-cli behavior.
...
The HuggingFace CLI uses specific exit codes to indicate the reason for
termination. This is especially useful for scripting and automation.
...
guide_, consider searching the HuggingFace CLI
[Issue tracker on GitHub](https://github.com/huggingface/hf-cli/issues). If you
