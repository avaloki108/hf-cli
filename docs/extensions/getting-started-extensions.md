# Getting started with HuggingFace CLI extensions

This guide will walk you through creating your first HuggingFace CLI extension.
You'll learn how to set up a new extension, add a custom tool via an MCP server,
create a custom command, and provide context to the model with a `HF.md` file.

## Prerequisites

Before you start, make sure you have the HuggingFace CLI installed and a basic
...
This is the manifest file for your extension. It tells HuggingFace CLI how to
load and use your extension.
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP)
  ...
  - `command`, `args`, `cwd`: These fields specify how to start your server.
    Notice the use of the `${extensionPath}` variable, which HuggingFace CLI
...
link the extension to your HuggingFace CLI installation for local development.
...
3.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from the HuggingFace CLI
    extensions directory to your development directory. This means any changes
    you make will be reflected immediately without needing to reinstall.

    ```bash
    gemini extensions link .
    ```

Now, restart your HuggingFace CLI session. The new `fetch_posts` tool will be
...
After saving the file, restart the HuggingFace CLI. You can now run
`/fs:grep-code "some pattern"` to use your new command.

## Step 5: Add a custom `HF.md`

You can provide persistent context to the model by adding a `HF.md` file to your
extension. This is useful for giving the model instructions on how to behave or
information about your extension's tools. Note that you may not always need this
for extensions built to expose commands and prompts.

1.  Create a file named `HF.md` in the root of your extension directory:
...
Restart the CLI again. The model will now have the context from your `HF.md`
...
You've successfully created a HuggingFace CLI extension! You learned how to:
...
capabilities into the HuggingFace CLI.
