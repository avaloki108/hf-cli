...
which projects can use the full capabilities of the HuggingFace CLI. It prevents
...
Once the feature is enabled, the first time you run the HuggingFace CLI from a
...
When a folder is **untrusted**, the HuggingFace CLI runs in a restricted "safe
...
Granting trust to a folder unlocks the full functionality of the HuggingFace CLI
for that workspace.
