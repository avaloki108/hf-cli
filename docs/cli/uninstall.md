...
"uninstall" the CLI, you must clear this cache, which will remove hf-cli and any
...
If you installed the CLI globally (e.g., `npm install -g @huggingface/hf-cli`),
use the `npm uninstall` command with the `-g` flag to remove it.

```bash
npm uninstall -g @huggingface/hf-cli
```
