...
This monorepo contains two main packages: `@huggingface/hf-cli` and
`@huggingface/hf-cli-core`.

## `@huggingface/hf-cli`

This is the main package for the HuggingFace CLI. It is responsible for the user
...
`@huggingface/hf-cli-core`. This means that whether a user installs the package
with `npm install -g @huggingface/hf-cli` or runs it directly with
`npx @huggingface/hf-cli`, they are using this single, self-contained
executable.
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @huggingface/hf-cli`.
