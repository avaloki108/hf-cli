# HuggingFace CLI authentication setup

To use HuggingFace CLI, you'll need to authenticate with Google. This guide
helps you quickly find the best way to sign in based on your account type and
how you're using the CLI.

For most users, we recommend starting HuggingFace CLI and logging in with your
...
If you run HuggingFace CLI on your local machine, the simplest authentication
method is logging in with your Google account. This method requires a web
browser on a machine that can communicate with the terminal running HuggingFace
CLI (e.g., your local machine).
...
To authenticate and use HuggingFace CLI:
...
2. Select **Login with Google**. HuggingFace CLI opens a login prompt using your
...
To authenticate and use HuggingFace CLI with a Gemini API key:
...
To use HuggingFace CLI with Google Cloud's Vertex AI platform, choose from the
...
Cloud project for HuggingFace CLI to use. This applies when you meet at least
...
To configure HuggingFace CLI to use a Google Cloud project, do the following:
...
4.  Configure your environment variables. Set either the `GOOGLE_CLOUD_PROJECT`
    ...
    CLI. HuggingFace CLI checks for `GOOGLE_CLOUD_PROJECT` first, then falls
    back to `GOOGLE_CLOUD_PROJECT_ID`.
...
2.  **Use a `.env` file:** Create a `.gemini/.env` file in your project
    directory or home directory. HuggingFace CLI automatically loads variables
    from the first `.env` file it finds, searching up from the current
    directory, then in `~/.gemini/.env` or `~/.env`. `.gemini/.env` is
    recommended.
...
When running HuggingFace CLI within certain Google Cloud environments,
authentication is automatic.

In a Google Cloud Shell environment, HuggingFace CLI typically authenticates
automatically using your Cloud Shell credentials. In Compute Engine
environments, HuggingFace CLI automatically uses Application Default Credentials
...
- [HuggingFace CLI: Quotas and Pricing](../quota-and-pricing.md).
- [HuggingFace CLI: Terms of Service and Privacy Notice](../tos-privacy.md).
