...
> `https://raw.githubusercontent.com/huggingface/hf-cli/main/schemas/settings.schema.json`.
...
- **`ui.showStatusInTitle`** (boolean):
  - **Description:** Show HuggingFace CLI status and thoughts in the terminal
    window title
  - **Default:** `false`
...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (HF.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory refresh loads HF.md files. When true,
...
- **`tools.enableHooks`** (boolean):
  - **Description:** Enable the hooks system for intercepting and customizing
    HuggingFace CLI behavior. When enabled, hooks configured in settings will
...
discovering and using custom tools. HuggingFace CLI attempts to connect to each
...
Configures logging and metrics collection for HuggingFace CLI. For more
...
(defaulting to `HF.md` but configurable via the `context.fileName` setting) are
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  ...
  1.  **Global context file:**
      - Location: `~/.gemini/<configured-context-filename>` (e.g.,
        `~/.gemini/HF.md` in your user home directory).
