...
This document describes the `run_shell_command` tool for the HuggingFace CLI.
...
## How to use `run_shell_command` with the HuggingFace CLI
...
`settings.json` file or by using the `/settings` command in the HuggingFace CLI.
...
tools to detect if they are being run from within the HuggingFace CLI.
